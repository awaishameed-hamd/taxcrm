...
export default function ClientFormSettings() {
  ...
  const loadFields = useCallback(async () => {
    setLoading(true)
    try {
      const res  = await api.get('/crm/form-fields', { params: { form_type: 'client' } })
  ...
  const handleAddField = async (fieldData: Record<string, any>) => {
    try {
      const res     = await api.post('/crm/form-fields', { ...fieldData, form_type: 'client' })
  ...
  const handleDeleteField = async (fk: string) => {
    ...
    try {
      await api.delete(`/crm/form-fields/client/${fk}`)
