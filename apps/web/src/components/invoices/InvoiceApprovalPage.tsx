      ...
      <DataTable
        ...
        columns={[
          ...
          { key: 'actions', label: '', width: 243, resizable: false, cellStyle: { overflow: 'visible' }, render: (r: any) => {
            ...
            const isContract = r.kind === 'RETAINER' || r.kind === 'ANNUAL'
            return (
                    <div style={{ display: 'flex', gap: 5, justifyContent: 'flex-end' }}>
                      ...
                      {/* Marking work as covered by a contract only makes sense
                          for a task draft. A retainership invoice IS the contract
                          being billed, so it cannot be absorbed into itself. */}
                      {!isContract && (
                        <>
                          <button onClick={() => act(r.id, 'mark-retainer')} disabled={disabled} title="Covered by the monthly retainership, don't bill separately"
                          ...
                          <button onClick={() => act(r.id, 'mark-annual')} disabled={disabled} title="Covered by the annual retainership, don't bill separately"
