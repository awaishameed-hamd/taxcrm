...
function openFile(url: string, type: string) {
  const inline = type === 'pdf' || type === 'image'
  const full = url.startsWith('http') ? url : fileHref(url, { download: !inline })
  if (inline) { window.open(full, '_blank') }
  else { const a = document.createElement('a'); a.href = full; a.click() }
}
        ...
        <div style={{ minWidth: 280, display: 'flex', flexDirection: 'column', height: '100%' }}>
          <div style={{ flexShrink: 0, borderBottom: `1px solid ${P.border}` }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 52, padding: '0 14px' }}>
              <h2 style={{ margin: 0, fontFamily: "'Faster One', cursive", textTransform: 'uppercase', fontSize: 18, display: 'inline-block', whiteSpace: 'nowrap', color: '#1E8496' }}>Files</h2>
