            ...
            {!cLoading && filtered.map(c => {
              ...
              return (
                <button key={c.id} onClick={() => { setSelected(c); setOpenMonth(null); setOpenFolder(null) }}
                  style={{ display: 'block', width: '100%', textAlign: 'left', padding: '7px 14px', border: 'none', cursor: 'pointer', borderBottom: `1px solid ${P.border}`, background: isActive ? '#E8EEF7' : '#F8FAFC', borderLeft: isActive ? `3px solid ${TEAL}` : '3px solid transparent' }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 700, color: isActive ? TEAL : NAVY, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{displayName(c)}</span>
                  </div>
                </button>
              )
            })}
